...
function genericErr(statusCode, msg) {
    return {
        statusCode,
        headers: { 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ status: 'err', msg })
    };
}
...
exports.handler = (event, context, callback) => {
  try { 
    ...
    if (typeof auth_header !== 'string' || auth_header.split('.').length !== 3) {
        return callback(null, genericErr(401, 'missing or malformed authorization'));
    }
    ...
    try {
      ...
      console.error('Unhandled exception in order-manager:', e);
      return callback(null, genericErr(500, 'internal error'));
  }
};
