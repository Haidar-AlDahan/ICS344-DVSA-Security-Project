...
def updateItem(orderId, user, obj, ts):
    update_expr = (
        "SET itemList = :itemList, orderStatus = :orderStatus, "
        "address = :address, confirmationToken = :token, paymentTS = :ts, "
        "totalAmount = :total"
    )
    table.update_item(
...
def lambda_handler(event, context):
    ...
    # Lesson 5 fix: normal users must not perform admin order updates.
    # Only tokens with the admin custom attribute may continue.
